...
{
    public class BlockSlotTests : IDisposable
    {
        ...
        [Fact]
        public async Task BlockSlotAsync_ValidRequest_BlocksSlot()
        {
            // Arrange
            ...
            // Act
            var result = await _slotService.BlockSlotAsync(slotId, userId, tenantId);

            // Assert
            Assert.True(result);

            // Verify that the slot was blocked
            var blockedSlot = await _dbContext.Slots.FindAsync(slotId);
            Assert.NotNull(blockedSlot);
            Assert.False(blockedSlot.IsAvailable);
        ...
        [Fact]
        public async Task BlockSlotAsync_UnauthorizedUser_ThrowsUnauthorizedAccessException()
        {
            // Arrange
            var providerId = Guid.NewGuid();
            ...
            await Assert.ThrowsAsync<UnauthorizedAccessException>(() => 
                _slotService.BlockSlotAsync(slotId, userId, tenantId));
        }
    }
}