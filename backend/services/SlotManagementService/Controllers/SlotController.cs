...
namespace SlotManagementService.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class SlotController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateRecurringSlots([FromBody] CreateRecurringSlotsRequest request)
        {
            try
            {
                ...
                _logger.LogInformation($"Created {slots.Count()} recurring slots");
                
                return Ok(new { Count = slots.Count(), Message = "Recurring slots created successfully" });
            }
            catch (ArgumentException ex)
            {
                return BadRequest(new { Message = ex.Message });
