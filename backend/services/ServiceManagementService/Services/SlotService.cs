...
namespace ServiceManagementService.Services
{
    public class SlotService : ISlotService
    {
        ...
        public SlotService(ApplicationDbContext dbContext, ISlotValidator validator, IPublishEndpoint publishEndpoint)
        {
        ...
        public async Task<Slot> CreateSlotAsync(Shared.DTOs.CreateSlotRequest request, Guid userId, Guid tenantId)
        {
        ...
        {
            // Validate the request
            var validationResult = await _validator.ValidateUpdateSlotRequestAsync(request);
            if (!validationResult.IsValid)
            {
                throw new ArgumentException(string.Join("; ", validationResult.Errors));
            }
        ...
        public async Task<IEnumerable<Slot>> CreateRecurringSlotsAsync(Shared.DTOs.CreateRecurringSlotsRequest request, Guid userId, Guid tenantId)
        {
            // Validate the request
            ...
            while (currentDate <= endDate && occurrences < request.Occurrences)
            {
                ...
                {
                    case Shared.DTOs.RecurrencePattern.Daily:
                        currentDate = currentDate.AddDays(request.Interval);
                        break;
                    case Shared.DTOs.RecurrencePattern.Weekly:
                        currentDate = currentDate.AddDays(7 * request.Interval);
                        break;
                    case Shared.DTOs.RecurrencePattern.Monthly:
